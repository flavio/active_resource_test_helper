...
Usually ActiveResouce tests looks like that:
...
While using active_resource_test_helper they would be something like that:
...
Currently the following operations are fully supported:
...
In order to use active_resource_test_helper inside of your test you have to:

* define the factories used by ActiveResource tests (see below)
...
Factories used by ActiveResource::HttpMock are defined using the _Factory.define_active_resource_factory_ method:
...
This methods uses the same syntax of factory_girl[http://github.com/thoughtbot/factory_girl].
...
Currently redis doesn't have a complete transaction support. Fortunately _active_resource_test_helper_ removes all the models defined by the ActiveResource factories used by the test from redis database. This operation is executed before and after each test execution.

== Statically defined responses

It's possible to use dynamic contents and static pairs of request-response at the same time.
However the responses defined in the static way have precedence over the ones generated in the dynamic way.